...
import { Code, Tabs, Steps, TabItem } from '@astrojs/starlight/components';
...
If you have feedback, questions, or want to contribute to Aspire, collaborate with us on [<Icon name="github" class='d-inline' /> GitHub](https://github.com/dotnet/aspire) or join us on our new [<Icon name="discord" class='d-inline' /> Discord](https://aka.ms/aspire-discord) to chat with the team and other community members.
...
```csharp title="C# — Flexible Python application models"
var builder = DistributedApplication.CreateBuilder(args);
...
```csharp title="C# — Uvicorn integration"
...
```csharp title="C# — Automatic package detection"
...
```csharp title="C# — Using uv for package management"
...
```csharp title="C# — Using pip for package management"
builder.AddUvicornApp("api", "./api", "main:app")
...
```csharp title="C# — Configuring virtual environment path"
...
```csharp title="C# — Automatic Dockerfile generation"
// With uv (recommended)
builder.AddUvicornApp("api", "./api", "main:app")
    .WithUv();
...
```bash title="Bash — Create Python starter template"
...
```csharp title="C# — Unified JavaScript application model"
var builder = DistributedApplication.CreateBuilder(args);
...
```csharp title="C# — Customizing package managers"
...
```csharp title="C# — Customizing scripts"
...
```csharp title="C# — Pass arguments using WithArgs"
...
```csharp title="C# — Reference custom script"
// Before (Aspire 9.x with AddNpmApp)
// builder.AddNpmApp("hasura-console", "../Web/ClientApp", 
//     scriptName: "hasura:console", 
//     args: ["--no-browser"])
...
```csharp title="C# — Dynamic Dockerfile generation"
var app = builder.AddJavaScriptApp("app", "./app");
```
...
```csharp title="C# — Polyglot connection properties"
...
```csharp title="C# — Certificate trust across languages"
...
```csharp title="C# — Simplified service URL environment variables"
var builder = DistributedApplication.CreateBuilder(args);
...
```bash title="Bash — Initialize new Aspire solution"
...
```bash title="Bash — Aspire update commands"
...
```csharp title="C# — Single-file AppHost"
// apphost.cs
#:sdk Aspire.AppHost.Sdk@13.0.0
...
```csharp title="C# — Custom pipeline step"
var builder = DistributedApplication.CreateBuilder(args);
...
```bash title="Bash — Run custom pipeline step"
aspire do validate
```
...
```csharp title="C# — Container files as build artifacts"
var frontend = builder.AddViteApp("frontend", "./frontend");
var api = builder.AddUvicornApp("api", "./api", "main:app");
...
```python title="Python — Serving front end from back end"
...
<Image src={languageIcons} alt="Polyglot language icons" title="Polyglot language icons" />
